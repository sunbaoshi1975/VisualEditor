...
Begin VB.Form frm_050 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cb_FileNameType 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "frm_050.frx":2CE0
         Left            =   2400
         List            =   "frm_050.frx":2CE2
         Style           =   2  'Dropdown List
         TabIndex        =   5
         ToolTipText     =   "请选择"
         Top             =   1500
    ...
    If f_DataChanged Then
        ...
        Else
            If Val(Cb_timeout) > 3600 Then
                ...
                Exit Sub
            Else
                Node50_Data1.timeout = CByte(Val(Cb_timeout.Text) Mod 256)
            End If
        End If
        ...
        Else
           Node50_Data1.trytimes = CByte(Cb_maxtrytime.ItemData(Cb_maxtrytime.ListIndex))
        End If
        
        ' 文件名类型
        If cb_FileNameType.ListIndex < 0 Then
           Node50_Data1.filenametype = DEF_NODE050_FAXN_TYPE_RESID
        Else
           Node50_Data1.filenametype = CByte(cb_FileNameType.ItemData(cb_FileNameType.ListIndex))
        End If
        ...
        Else
            Node50_Data1.var_faxfile = CByte(Cb_Var_FileName.ItemData(Cb_Var_FileName.ListIndex))
        End If
        ...
        Else
            Node50_Data1.var_fromno = CByte(Cb_Var_Sender.ItemData(Cb_Var_Sender.ListIndex))
        End If
        ...
        Else
            Node50_Data1.var_tono = CByte(Cb_Var_Receiver.ItemData(Cb_Var_Receiver.ListIndex))
        End If
        ...
        Else
            Node50_Data1.var_result = CByte(Cb_var_result.ItemData(Cb_var_result.ListIndex))
        End If
        ...
        If Cb_log.ListIndex < 0 Then
            Node50_Data1.log = 0
        Else
            Node50_Data1.log = CByte(Cb_log.ItemData(Cb_log.ListIndex))
        End If

        ' 变量：附加字段
        For lv_loop = 0 To 2
            If Cb_VarAppField(lv_loop).ListIndex <= 0 Then
                Node50_Data1.var_appfield(lv_loop) = 0
            Else
                Node50_Data1.var_appfield(lv_loop) = CByte(Cb_VarAppField(lv_loop).ItemData(Cb_VarAppField(lv_loop).ListIndex))
            End If
        Next
        ...
        Else
            If CLng(Trim(T_fax_fileid)) > 32767 Then
                ...
                Exit Sub
            Else
                Node50_Data2.fax_fileid = CInt(Trim(T_fax_fileid.Text))
            End If
        End If
        ...
        Else
            If CLng(Trim(txtHeader)) > 32767 Then
                ...
                Exit Sub
            Else
                Node50_Data2.header_id = CInt(Trim(txtHeader))
            End If
        End If
        ...
        Else
            If CLng(Trim(T_vox_op)) > 32767 Then
                ...
                Exit Sub
            Else
                Node50_Data2.vox_op = CInt(Trim(T_vox_op))
            End If
        End If
        ...
        Else
            If (Val(T_nd_parent) > 32767 Or Val(T_nd_parent) < 256) And Val(T_nd_parent) <> 0 Then
                ...
                Exit Sub
            Else
                Node50_Data2.nd_parent = CInt(Trim(T_nd_parent.Text))
            End If
        End If
        ...
        If Node50_Data2.nd_succ <> lv_nNewNode Then
            Node50_Data2.nd_succ = lv_nNewNode
        ...
        '' Sun added 2007-03-25
        If Node50_Data2.nd_fail <> lv_nNewNode Then
            Node50_Data2.nd_fail = lv_nNewNode
            Call gCallFlow.AutoChangeLineIndex(CByte(T_n_no), CInt(T_n_id), lv_nNewNode, 0)
        End If
        
        ' 记录传真详单
        Node50_Data1.record_cdr = chkRecord_cdr
...
SetMainFormItemsEnableWhenPropertyShow False
    ...
    With cb_FileNameType
        .Clear
        For i = DEF_NODE050_FAXN_TYPE_RESID To DEF_NODE050_FAXN_TYPE_FORMAT
    ...
    ' 操作提示音
    T_vox_op.Text = Node50_Data2.vox_op
    
    ' 传真文件
    T_fax_fileid.Text = Node50_Data2.fax_fileid
    
    ' 传真标题资源ID
    txtHeader = Node50_Data2.header_id
    
    ' 父节点ID
    T_nd_parent.Text = Node50_Data2.nd_parent
    
    ' 成功转节点ID
    T_nd_succeed.Text = Node50_Data2.nd_succ
    
    ' 失败转节点ID
    T_nd_fail.Text = Node50_Data2.nd_fail
    
    ' 记录传真详单
    chkRecord_cdr = Node50_Data1.record_cdr
    
    ' 描述
    Txt_Description.Text = gCallFlow.Node(gCallFlow.NodeSelectedID).Description
    
    Cb_timeout.Text = Node50_Data1.timeout
    Cb_maxtrytime.ListIndex = SearchItemDataIndex(Cb_maxtrytime, CLng(Node50_Data1.trytimes), 3)
    Cb_log.ListIndex = SearchItemDataIndex(Cb_log, CLng(Node50_Data1.log), 0)
    
    cb_FileNameType.ListIndex = SearchItemDataIndex(cb_FileNameType, CLng(Node50_Data1.filenametype), 1)
    Cb_Var_FileName.ListIndex = SearchItemDataIndex(Cb_Var_FileName, CLng(Node50_Data1.var_faxfile), 0)
    Cb_Var_Sender.ListIndex = SearchItemDataIndex(Cb_Var_Sender, CLng(Node50_Data1.var_fromno), 0)
    Cb_Var_Receiver.ListIndex = SearchItemDataIndex(Cb_Var_Receiver, CLng(Node50_Data1.var_tono), 0)
    Cb_var_result.ListIndex = SearchItemDataIndex(Cb_var_result, CLng(Node50_Data1.var_result), 0)
    
    Cb_VarAppField(0).ListIndex = SearchItemDataIndex(Cb_VarAppField(0), CLng(Node50_Data1.var_appfield(0)), 0)
    Cb_VarAppField(1).ListIndex = SearchItemDataIndex(Cb_VarAppField(1), CLng(Node50_Data1.var_appfield(1)), 0)
    Cb_VarAppField(2).ListIndex = SearchItemDataIndex(Cb_VarAppField(2), CLng(Node50_Data1.var_appfield(2)), 0)
...
Private Sub T_fax_fileid_Change()
    ...
    F_RefreshVoxBoxToolTip T_fax_fileid
